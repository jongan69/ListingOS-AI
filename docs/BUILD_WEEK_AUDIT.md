...
| --- | --- | --- | --- |
| Repo access | Green | `https://github.com/jongan69/ListingOS` is public, `main` is the default branch, and the repository already includes an MIT license and submission-oriented docs. | Keep the repo public through judging, or if you later make it private, share it with `testing@devpost.com` and `build-week-event@openai.com` before the deadline. |
