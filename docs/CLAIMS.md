...
| Sony remote camera control | Android and iOS preview-gated Camera Control PTP 2 modules implement USB attach detection, native JPEG live view, shutter control, and JPEG transfer for the A7 III. Static/native compilation is required, and physical camera acceptance has not passed on either platform. | High | No | Tethered Sony control is an internal hardware-validation spike. Do not imply the app operates the camera until physical evidence passes. |
