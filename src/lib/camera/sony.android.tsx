...
export function useSonyCameraConnection(enabled = true): SonyCameraConnection {
  ...
  useEffect(() => {
    if (!__DEV__ || !available) return;
    const device = state.device;
    const latestDiagnostic = state.diagnostics?.[state.diagnostics.length - 1];
    console.info(
      `[SonyCamera] state=${state.state}` +
        (device ? ` device=${device.vendorId.toString(16)}:${device.productId.toString(16)}` : "") +
        (state.message ? ` message=${state.message}` : "") +
        (latestDiagnostic ? ` trace=${latestDiagnostic}` : ""),
    );
  }, [available, state]);
  ...
  return useMemo(() => ({
    ...
    connect,
    capturePhoto,
  }), [available, capturePhoto, connect, state]);
}
